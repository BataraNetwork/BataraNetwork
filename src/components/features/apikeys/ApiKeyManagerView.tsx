...
import { PlusCircleIcon, TrashIcon, ClipboardIcon, CheckIcon } from '../../ui/icons';
...
const ApiKeyRow: React.FC<{ apiKey: ApiKey, onRevoke: (id: string) => void }> = ({ apiKey, onRevoke }) => {
    return (
        <tr className="border-b border-slate-700 last:border-b-0">
            ...
            <td className="p-4">
                <button onClick={() => onRevoke(apiKey.id)} className="text-red-400 hover:text-red-300 flex items-center gap-1 text-sm">
                    <TrashIcon /> Revoke
                </button>
            </td>
        </tr>
...
export const ApiKeyManagerView: React.FC = () => {
    ...
    const handleCreateKey = (e: React.FormEvent) => {
        ...
        if (keyName.trim()) {
            ...
            createKey(keyName, ['read:all', 'write:transactions']);
            setKeyName('');
        }
    };
    ...
    return (
        <div>
            ...
            <form onSubmit={handleCreateKey} className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 mb-8 flex flex-col sm:flex-row gap-4 items-center">
                ...
                />
                 <button type="submit" disabled={!keyName.trim()} className="w-full sm:w-auto bg-sky-600 text-white font-semibold rounded-md px-4 py-2 hover:bg-sky-500 transition-colors flex items-center justify-center gap-2 disabled:bg-slate-600 disabled:cursor-not-allowed">
                    <PlusCircleIcon />
                    Create New Key
                </button>
            </form>
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-lg overflow-x-auto">
                <table className="min-w-full">
                    ...
                    <tbody>
                        {keys.map(key => <ApiKeyRow key={key.id} apiKey={key} onRevoke={revokeKey} />)}
                    </tbody>
                </table>
